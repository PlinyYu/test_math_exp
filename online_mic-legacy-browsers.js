...
function mic_accessRoutineBegin(snapshot) {
  return async function () {
    ...
    mic_accessComponents.forEach( function(thisComponent) {
...
function preludeRoutineBegin(snapshot) {
  return async function () {
    ...
    preludeComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function never_ending_routineRoutineBegin(snapshot) {
  return async function () {
    ...
    never_ending_routineComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
